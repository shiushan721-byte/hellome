...
export default function InfoSections() {
  ...
  const faqs: FaqItem[] = [
    ...
    {
      question: 'Hz-Hermes 引擎是如何保证数据分析和执行效率的？',
      answer: 'Hz-Hermes 是 HelloMe 专有的轻量级代理流程引擎。它能将复杂的业务流程分解为特定的 Skills（如获取网络上下文，执行大模型逆向推荐评估，生成工单策略等），并进行实时 DAG 工作流编排。通过内置异步执行策略与本地备用缓存，一个高精度的全渠道 AI 提及率分析仅需 4.5 秒即可产出精细日志。'
    },
    {
