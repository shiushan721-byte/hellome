...
## 2026-06-21 视频智能体工作台业务驱动重构

### 本轮完成

- 重构 [src/pages/app/UgcVideoAgentPage.tsx](/Users/feihong/Documents/hellome/src/pages/app/UgcVideoAgentPage.tsx)
- 将原来的“上传素材 + 大输入框”工作台，改成更贴近确认后的业务导向结构：
  - 左栏：业务项选择 + 素材上传 + 系统整理的业务描述
  - 中栏：结果预览 + 内部参数预览卡
  - 右栏：案例、产出、预算与任务说明
- 去掉了用户侧重复选择视频类型的逻辑：
  - 页面内只展示“当前智能体方向”
  - 视频方向不再在执行页重复暴露为可选项
- 为 6 个视频智能体补上更贴近业务的选项分组：
  - 新品种草视频
  - 测评讲解视频
  - 带货转化视频
  - 品牌宣传视频
...
- 保留现有后端接口不变：
  - 前端先把业务选项自动整理成业务描述
  - 再沿用现有 `createRemoteUgcTask()` 创建任务

### 当前收益

- 用户端页面不再要求小白直接写 Prompt。
- 视频智能体进入页后，已经更接近“选业务 -> 看系统理解 -> 确认开始”的真实交付逻辑。
- 用户侧不再暴露 Hermes 执行细节，而是只看到结果预览、案例和预算说明。

### 本轮验证

- `npm run build` 已通过
- 当前仍保留既有构建警告：
  - CSS 中已有的 `"file" is not a known CSS property`
  - 打包体积超过 `500 kB`
- 尝试做本地自动截图时，发现当前环境缺少 Playwright 浏览器内核，未完成自动截图验证

### 二次收敛

- 继续收紧使用页的前置输入逻辑：
  - 在业务选项尚未补齐前，“系统整理的业务描述”不再直接显示为可编辑输入框
  - 改为只读提示卡，强调“先选业务，系统再整理”
- 在中栏补充轻量三阶段提示：
  - 先选业务
  - 系统整理方向
  - 确认后生成样片
- 这组三阶段提示不暴露执行细节，只用于帮助普通用户理解自己当前所处位置

### 三次收敛

- 收紧 `/app/tasks/:id` 视频任务执行页的普通用户视角：
  - 普通用户在“修改需求后重新生成”中不再看到平台、效果目标等固定参数
  - 仅允许修改业务描述与补充参考
- 执行页中的输入摘要改成更业务化表述：
  - `系统整理后的任务内容`
  - `业务描述`
  - `固定规格`
  - `当前表达方向`
- `TaskRunLayout` 的视频任务顶部与侧栏文案同步收敛：
  - `视频智能体任务`
  - `视频任务执行`
  - `任务进度`
  - `补充说明`
...
- 用户点击纠偏入口后：
  - 自动展开“修改需求后重新生成”
  - 自动把修正意图写入业务描述编辑区
- 这样普通用户不需要从零组织返工语言，就能直接按业务方向发起下一轮生成

### 五次收敛

- 将视频智能体的市场入口承诺统一收口到 `videoAgentProfiles`：
  - `适合谁`
  - `需要输入什么`
  - `最终交付物`
  - `市场入口 CTA`
- 市场卡片、市场中 banner、公开详情页现在都从同一份视频智能体配置读取文案
- 视频智能体市场 CTA 不再统一显示“使用智能体”，而是按业务语义动态变化：
  - `开始做种草视频`
  - `开始做讲解视频`
  - `开始做带货视频`
  - `开始做宣传视频`
...
- 上传卡现在具备更完整的真实前端状态：
  ...
  - 已上传
  - 上传失败
- 普通用户在使用页里对素材状态的感知更清晰，后续再接真实存储和更严格校验时也更容易延展
...
## 2026-06-20 视频智能体工作台前台骨架收敛

### 本轮完成

- 重构了 [src/pages/app/UgcVideoAgentPage.tsx](/Users/feihong/Documents/hellome/src/pages/app/UgcVideoAgentPage.tsx)
- 本轮不是继续在旧 demo 样式上修补，而是按已经确认的前台规范，把页面从“页面内部再做一套 left / center / right”收回成平台级工作台逻辑：
  - 左侧全局导航继续由 App Shell 承担
  - 页面内部只保留：
    - 左侧业务输入区
    - 右侧结果舞台与按需展开信息区
- 重点收敛了以下旧问题：
  - 删除开发态英文区块标题，如 `Left / Center / Right / Progress`
  - 删除旧版顶部“类型 / 格式 / 运行”信息拼盘
  - 删除未开始前直接暴露的大段交付文件清单
  - 删除把 Hermes 状态、账单、运行说明都堆在主交易区的旧 demo 表达
- 同时把新的前台交互逻辑落进页面：
  - 输入区只保留最少必要输入
  - `参考链接 / 输出设置` 收进 `更多设置`
  - 主按钮统一收敛为 `开始生成样片`
  - 右侧结果区默认先展示结果舞台
  - `预估成本 / 交付说明 / 执行过程` 全部改成按需展开
  - 专业信息与本地 Hermes 调试只在展开层里给 creator 使用
...
- 单独用 `tsc --noEmit src/pages/app/UgcVideoAgentPage.tsx` 的方式检查会受到项目当前 TypeScript CLI 配置方式影响，不适合作为单文件校验手段；以整项目 `build` 通过为准

### 当前收益

- 工作台首页终于从“解释系统怎么工作”收回到了“让用户尽快开始并先看结果方向”。
- 这页现在更接近后续真实任务系统接入后的正式前台，而不是早期演示拼贴页。

### 二次收敛

- 继续按业务逻辑收掉了执行页里的重复决策：
  - 不再让用户在执行页重新选择“发布平台 / 目标效果 / 视频方向”
  - 改成由外部已选中的智能体预设直接锁定
- 对应改动落在：
  - [src/pages/app/UgcVideoAgentPage.tsx](/Users/feihong/Documents/hellome/src/pages/app/UgcVideoAgentPage.tsx)
- 当前执行页已调整为：
  - 用户只补素材与一句话卖点
  - 视频方向、平台、规格、交付方式改为只读预设
  - 结果预览不再提供“换个方向看看”的二次选择
- 当前实现中，三个视频智能体的执行预设已先做成固定映射：
  - `真人种草视频 -> 抖音 / 更像真人种草`
  - `测评讲解视频 -> 视频号 / 更像测评讲解`
  - `带货转化视频 -> 抖音 / 更像带货转化`
...
- 前台执行页终于和“外部先选智能体、内部再执行任务”的业务链路对齐了。
- 用户不会再在进入智能体后被迫重新回答一遍“我到底要什么视频”。

### 三次收敛

- 继续把执行页从“配置回显页”收成“智能体承诺页”：
  - 顶部副文案优先读取智能体的 `promiseLine`
  - 左侧原本像配置卡的“已锁定信息”改成更轻的只读承诺卡
  - 删除整块 `当前选择` 汇总区，避免再次重复同一组预设信息
...
## 2026-06-21 我的工作台视频智能体使用页收敛为任务投递页

### 本轮完成

- 重构 `/app/agents/media-*` 使用页的信息架构，不再把页面做成“参数配置后台”：
  - 左侧主区改为 `先上传素材 / 本次任务内容`
  - 中间主输入改成 `告诉我这次想解决什么业务问题`
  - 右侧统一改为 `结果舞台`
- 补齐了普通用户真实会经历的两种提交前状态：
  - `输入前空态`
  - `输入后未开始态`
- 把右侧从“多方向候选预览”收敛成单一结果线：
  - 输入前只展示默认案例气质
  - 输入后只展示本次任务预期
  - 不再暗示还可以换方向、换风格、看别的路线
- 去掉普通用户首屏中的技术执行痕迹：
  - 移除了 Hermes 状态、路由策略、调试输出、执行步骤列表
  - 不再把 `后端静默执行 / 本地调试模式` 暴露在用户页头部
- 收紧参数层级：
  - 保留固定规格提示，但不再混入执行策略
  - `先出样片后确认` 改为任务说明中的策略文案，而不是规格标签
- 强化上传交互：
  - 上传卡区分未上传与已上传状态
  - 已上传态显示文件名与“替换素材”动作
  - 人物图明确为可选补充，而不是强制项
- 底部主按钮只在核心输入完整后可点，强化“已填写待开始”的任务感
...
- 用户进入页面后更容易理解：
  - 左边是这次要提交什么
  - 右边是这次大概会得到什么
  - 底部按钮是开始任务，而不是提交一堆系统配置
- 使用页现在比 Creator 页更轻，职责也更清晰：
  - Creator 页负责定义智能体
  - 使用页负责发起一次具体任务
- 这为后续继续补 `执行中状态页` 和 `样片结果态` 提供了稳定骨架。
...
- 构建阶段仍存在现有的 CSS 警告：`"file" is not a known CSS property`，不是本轮页面重构引入的新问题。

## 2026-06-21 视频任务执行页去后台化收口

### 本轮完成

- 收紧视频任务执行页语言和信息层级，普通用户不再首屏看到运行方式与调试语义。
- `TaskShowcaseStage` 的运行中文案从技术执行改成业务推进表达，不再直接暴露 Hermes。
- `UgcDeliveryPanel` 新增执行中业务提示卡，弱化“执行摘要”并改为“任务说明”。
- 纠偏 chips 仅在完成 / 待确认 / 失败时展示，不在纯执行中阶段打断用户。
- `TaskRunLayout` 对媒体任务做了更业务化标题收口，并把日志区收进创作者/专业视角。
...
- 构建阶段仍存在既有 CSS warning：`"file" is not a known CSS property`，不是本轮引入。

## 2026-06-21 6个业务向视频智能体前后映射补齐

### 本轮完成

- 新增共享配置 `src/config/videoAgentProfiles.ts`，统一定义 6 个视频智能体的对外包装名、工作台副标题、固定规格、默认平台/效果、输入字段文案。
- 市场与前台补齐 6 个视频智能体：新品种草视频、测评讲解视频、带货转化视频、品牌宣传视频、产品演示视频、客户提案视频。
- `UgcVideoAgentPage` 现在会按 agentId 动态切换输入提示、标题、副标题和规格标签。
- `skillStudioService` 的公开 skill experience 映射补齐到 6 个标准场景，并对齐到业务包装名。
- demo seed 同步使用对应智能体的 inputConfig 与 promiseLine，避免前后名字变了但字段没对上的假象。
- `ugcTaskService` 补齐 3 个新增视频智能体的任务显示名。
- App 路由补齐 `/app/agents/media-showcase`、`/app/agents/media-demo`、`/app/agents/media-proposal`。
...
- 当前仍保留“对外业务包装名 / 对内标准场景 id”的 B 方案，不改 contracts 主骨架。

## 2026-06-21 视频任务执行页继续业务化收敛

### 本轮完成

- `UgcDeliveryPanel` 继续按普通用户 / 创作者分层：普通用户默认看到业务目标、固定规格、当前表达方向，不再首屏看到脚本草案和目标用户等制作层信息。
- `TaskRunLayout` 为视频任务增加业务阶段标签映射：理解任务 / 整理表达方向 / 生成样片 / 整理交付。
- 修改后重新生成入口仅在更合理的阶段对普通用户开放，避免执行中阶段被“改配置”打断。
...
## 2026-06-21 视频智能体上传校验前后端对齐

### 本轮完成

- 收紧视频智能体使用页的素材上传规则，前后端统一为：
  - 仅支持 `JPG / PNG / WebP`
  - 单张图片不超过 `10MB`
- 前端在 [src/pages/app/UgcVideoAgentPage.tsx](/Users/feihong/Documents/hellome/src/pages/app/UgcVideoAgentPage.tsx) 增加上传前预校验：
  - 文件类型不符时直接提示
  - 文件过大时直接提示
  - 上传组件文案同步显示支持格式和大小上限
- 服务端在 [server.ts](/Users/feihong/Documents/hellome/server.ts) 为 `/api/uploads` 增加统一限制：
  - `multer` 文件大小限制
  - 图片 MIME 类型过滤
  - 结构化错误返回：
    - `图片不能超过 10MB`
    - `仅支持 JPG、PNG、WebP 图片`
    - `上传失败，请稍后重试`
- 上传卡与服务端错误文案已对齐，后续接真实对象存储时可以继续复用这一层规则。

### 当前收益

- 普通用户在上传前就能知道素材要求，不会等任务提交后才发现格式错误。
- 前端和后端的错误提示保持一致，避免“页面提示一套、接口报错另一套”的割裂体验。
- 为后续接真实视频素材、封面图和更多资产类型时，先建立了统一的校验入口。
